...
static VALUE wrap_runx_init(VALUE self, VALUE vfilename)
{
    ...
    menoh_model_data_handle model_data;
    ERROR_CHECK(
        menoh_make_model_data_from_onnx(filename, &model_data), rb_eArgError);
...
static VALUE wrap_model_run(VALUE self, VALUE dataset, VALUE condition)
{
    ...
    ERROR_CHECK(menoh_make_variable_profile_table_builder(&vpt_builder), rb_eStandardError);
    ...
    // optimize
    ERROR_CHECK(menoh_model_data_optimize(getModel(self)->model_data, variable_profile_table), rb_eStandardError);

    // get model buildler
    menoh_model_builder_handle model_builder;
    ERROR_CHECK(
        menoh_make_model_builder(variable_profile_table, &model_builder), rb_eStandardError);

    // attach input buffer to model builder
    float *input_buff;
    input_buff = (float *)malloc(sizeof(float) * batch_size * channel_num * width * height);
    ERROR_CHECK(menoh_model_builder_attach_external_buffer(model_builder, StringValuePtr(vinput_layer),
                                                           input_buff),
                rb_eStandardError);
    ...
    ERROR_CHECK(
        menoh_build_model(model_builder, getModel(self)->model_data, StringValuePtr(vbackend), "", &model), rb_eStandardError);
    ...
    for (int32_t output_layer_i = 0; output_layer_i < output_layer_num; output_layer_i++)
    {
        VALUE voutput_layer = rb_ary_entry(getModel(self)->voutput_layers, output_layer_i);
        ...
        rb_hash_aset(result_each, rb_to_symbol(rb_str_new2("shape")), vresult_shape);
        rb_hash_aset(result_each, rb_to_symbol(rb_str_new2("buffer")), vresult_buffer);
